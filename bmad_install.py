...
Exemplos:
  ...
  {os.path.basename(sys.argv[0])} --pin tea=v0.2.0         Fixar versão específica
    ...
    if success:
        ...
        log(f"\n   {Color.DIM}Próximos passos:{Color.RESET}")
        log(f"   {Color.DIM}  1. Abra seu projeto no Claude Code, Cursor ou outra IDE AI{Color.RESET}")
        log(f"   {Color.DIM}  2. Execute o workflow BMAD desejado{Color.RESET}")
